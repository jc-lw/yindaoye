...
# ============================================================================
# Standalone all-in-one workflow
# Combines the GCP/Vertex core library above with the useful orchestration from
# kn.sh. It does NOT download/source a remote test.sh at runtime.
# ============================================================================

# The library above enables strict mode. The original kn.sh intentionally ran its
# orchestration without errexit because many gcloud failures are classified and
# handled explicitly. Keep nounset + pipefail, but disable errexit/ERR trap here.
set +e
set +E
set -u
set -o pipefail
...
ALLINONE_VERSION="1.0.0"
NEED_PROJECTS="${NEED_PROJECTS:-2}"
REUSE_PROJECTS="${REUSE_PROJECTS:-1}"
REUSE_PROXY="${REUSE_PROXY:-1}"
BUILD_PROXY="${BUILD_PROXY:-1}"
FREE_TIER_REGIONS_ONLY="${FREE_TIER_REGIONS_ONLY:-1}"
FULL_API_PASS="${FULL_API_PASS:-0}"
NO_UPLOAD="${NO_UPLOAD:-1}"
PANEL_BASE="${PANEL_BASE:-https://kfc.v50.su}"
PANEL_TOKEN="${PANEL_TOKEN:-}"
UPLOAD_MODE="${UPLOAD_MODE:-pool}"
CREATED_BY="${CREATED_BY:-0}"
PROXY_PORT="${PROXY_PORT:-1080}"
PROXY_SOURCE_RANGES="${PROXY_SOURCE_RANGES:-0.0.0.0/0}"
PROXY_MACHINE_TYPE="${PROXY_MACHINE_TYPE:-e2-micro}"
PROXY_BOOT_DISK_SIZE="${PROXY_BOOT_DISK_SIZE:-10GB}"
PROXY_BOOT_DISK_TYPE="${PROXY_BOOT_DISK_TYPE:-pd-standard}"
PROXY_PROJECT_SCAN_LIMIT="${PROXY_PROJECT_SCAN_LIMIT:-12}"
PROXY_ZONE_TRIES="${PROXY_ZONE_TRIES:-8}"

# Faster orchestration defaults. Vertex SA propagation waits inside the library
# are intentionally left untouched.
PROJECT_SUBMIT_GAP="${PROJECT_SUBMIT_GAP:-1}"
PROJECT_CREATE_GAP="${PROJECT_CREATE_GAP:-1}"
API_BATCH_GAP="${API_BATCH_GAP:-0}"
API_REPAIR_ROUNDS="${API_REPAIR_ROUNDS:-2}"
API_REPAIR_SLEEP="${API_REPAIR_SLEEP:-3}"

say(){ echo -e "${CYAN}${BOLD}[allinone]${NC} $*"; }
ok(){ echo -e "${GREEN}[allinone] $*${NC}"; }
warn(){ echo -e "${YELLOW}[allinone] $*${NC}"; }
err(){ echo -e "${RED}[allinone] $*${NC}"; }

require_allinone_commands() {
  local missing=0 c
  for c in gcloud openssl curl python3 awk grep sed timeout; do
...
active_account() {
  local a
  a=$(gcloud auth list --filter=status:ACTIVE --format='value(account)' 2>/dev/null | head -n1)
  if [ -z "$a" ]; then
    a=$(gcloud config get-value account 2>/dev/null || true)
  fi
  [ "$a" = "(unset)" ] && a=""
  printf '%s\n' "$a"
}

choose_billing_account_auto() {
  local requested="${BILLING_ID:-}"
  if [ -n "$requested" ]; then
    requested="${requested#billingAccounts/}"
    printf '%s\n' "$requested"
    return 0
  fi

  local row bid
  row=$(billing_accounts_tsv 2>/dev/null | awk -F'\t' 'NF{print; exit}')
  [ -z "$row" ] && return 1
  bid=$(printf '%s\n' "$row" | awk -F'\t' '{print $1}')
...
list_reusable_billed_projects() {
  local limit="${1:-$NEED_PROJECTS}"
  local pid parent_type count=0

  while IFS= read -r pid; do
    [ -z "$pid" ] && continue

    # Keep the old no-org preference, but do not fail on accounts where parent
    # metadata is unavailable. If parent.type is organization/folder, skip it.
    parent_type=$(gcloud projects describe "$pid" --format='value(parent.type)' 2>/dev/null || true)
    if [ "$parent_type" = "organization" ] || [ "$parent_type" = "folder" ]; then
      continue
    fi
...
prepare_target_projects() {
  local billing_id="$1"
  local out_name="$2"
  local -n out_ref="$out_name"
  out_ref=()

  if [ "$REUSE_PROJECTS" = "1" ]; then
    say "扫描可复用且 billingEnabled=true 的项目..."
    local pid
    while IFS= read -r pid; do
      [ -z "$pid" ] && continue
      out_ref+=("$pid")
      say "  复用项目: $pid"
    done < <(list_reusable_billed_projects "$NEED_PROJECTS")
  fi

  local missing=$((NEED_PROJECTS - ${#out_ref[@]}))
  if [ "$missing" -gt 0 ]; then
    say "仍缺 ${missing} 个项目，调用 create_projects_exact 补建并绑定 Billing..."
    local new_pids=()
    create_projects_exact "$missing" "$billing_id" new_pids "allinone补建" || true
    local pid
    for pid in "${new_pids[@]:-}"; do
      ...
      if project_billing_enabled "$pid" 2>/dev/null; then
        ...
        warn "[$pid] Billing 尚未生效/绑定失败；Vertex 必需 API 仍会单独检测"
      fi
    done
  fi
...
# ---------- SOCKS5 proxy helpers ----------
proxy_free_tier_zones() {
  # Google Compute Engine Free Tier eligible regions are us-west1/us-central1/us-east1.
  # Zone names below remain inside those three regions.
  cat <<'ZONES'
us-west1-a
us-west1-b
us-west1-c
...
read_instance_proxy_metadata() {
  local project_id="$1" name="$2" zone="$3" ip="$4" out_file="$5"
  local mu mp mport

  mu=$(timeout 20 gcloud compute instances describe "$name" --project="$project_id" --zone="$zone" \
    --format='value(metadata.items.filter("key:kn-proxy-user").extract("value").flatten())' 2>/dev/null || true)
  mp=$(timeout 20 gcloud compute instances describe "$name" --project="$project_id" --zone="$zone" \
    --format='value(metadata.items.filter("key:kn-proxy-pass").extract("value").flatten())' 2>/dev/null || true)
  mport=$(timeout 20 gcloud compute instances describe "$name" --project="$project_id" --zone="$zone" \
    --format='value(metadata.items.filter("key:kn-proxy-port").extract("value").flatten())' 2>/dev/null || true)
  mport="${mport:-$PROXY_PORT}"

  [ -n "$mu" ] && [ -n "$mp" ] && [ -n "$ip" ] || return 1
  timeout 6 bash -c "cat < /dev/null > /dev/tcp/$ip/$mport" 2>/dev/null || return 1

  {
    printf 'PROXY_URL=socks5://%s:%s@%s:%s\n' "$mu" "$mp" "$ip" "$mport"
    printf 'PROXY_HOSTPORT=%s:%s\n' "$ip" "$mport"
    printf 'PROXY_ADSPOWER=%s:%s:%s:%s\n' "$ip" "$mport" "$mu" "$mp"
  } > "$out_file"
  return 0
}
...
find_reusable_proxy() {
  local out_file="$1"
  local scanned=0 pid line name zone ip

  say "[代理] 扫描现有 socks5-node VM，能复用则不创建新 VM..."
  while IFS= read -r pid; do
    [ -z "$pid" ] && continue
    scanned=$((scanned + 1))
    [ "$scanned" -gt "$PROXY_PROJECT_SCAN_LIMIT" ] && break

    line=$(timeout 20 gcloud compute instances list --project="$pid" \
      --filter='name~socks5-node AND status=RUNNING' \
      --format='value(name,zone,networkInterfaces[0].accessConfigs[0].natIP)' 2>/dev/null | head -n1 || true)
    [ -z "$line" ] && continue
    ...
    if read_instance_proxy_metadata "$pid" "$name" "$zone" "$ip" "$out_file"; then
      ok "[代理] 复用现有 VM: $name ($pid / $zone / $ip:$PROXY_PORT)"
      return 0
    fi
  done < <(gcloud projects list --format='value(projectId)' 2>/dev/null || true)
...
create_proxy_vm() {
  local project_id="$1" out_file="$2"
  local instance_name port proxy_user proxy_pass startup_file
  local zones=() zone tries=0 vm_ok=0 vm_out vm_rc external_ip

  port="$PROXY_PORT"
  instance_name="socks5-node-$(date +%s)-$RANDOM"
  proxy_user="usr$(openssl rand -hex 4)"
  proxy_pass="$(openssl rand -base64 18 | tr -dc 'a-zA-Z0-9' | head -c 18)"

  say "[代理] 使用项目: $project_id"
  say "[代理] 启用 Compute Engine API..."
  if ! gcloud services enable compute.googleapis.com --project="$project_id" --quiet >/dev/null 2>&1; then
    err "[代理] compute.googleapis.com 启用失败"
    return 1
  fi

  # Allow Service Usage propagation before the first instance operation.
  sleep 5

  if [ "$FREE_TIER_REGIONS_ONLY" = "1" ]; then
    mapfile -t zones < <(proxy_free_tier_zones | shuf)
    say "[代理] Free Tier 区域模式：只使用 us-west1/us-central1/us-east1"
  else
    mapfile -t zones < <(proxy_extended_zones | shuf)
    warn "[代理] 扩展区域模式：部分区域不属于 Compute Engine Free Tier"
  fi

  startup_file="/tmp/allinone_proxy_startup_$$.sh"
  cat > "$startup_file" <<VM_EOF
#!/bin/bash
set -e
...
VM_EOF
  ...
  for zone in "${zones[@]}"; do
    tries=$((tries + 1))
    [ "$tries" -gt "$PROXY_ZONE_TRIES" ] && break
    say "[代理] 创建 ${PROXY_MACHINE_TYPE}: $zone (尝试 $tries/$PROXY_ZONE_TRIES)"
    ...
    vm_out=$(gcloud compute instances create "$instance_name" \
      ...
      --boot-disk-type="$PROXY_BOOT_DISK_TYPE" \
      --boot-disk-size="$PROXY_BOOT_DISK_SIZE" \
    ...
    if printf '%s\n' "$vm_out" | grep -qiE 'ZONE_RESOURCE_POOL_EXHAUSTED|does not have enough resources|currently unavailable|resource pool'; then
      warn "[代理] $zone 暂无资源，自动换区域"
      continue
    fi

    err "[代理] 创建 VM 失败: $(printf '%s\n' "$vm_out" | tail -n 3 | tr '\n' ' ' | cut -c1-500)"
    rm -f "$startup_file"
    return 1
  done
  rm -f "$startup_file"

  [ "$vm_ok" -eq 1 ] || { err "[代理] 多个区域均创建失败"; return 1; }

  say "[代理] 创建/复用防火墙规则 allow-socks5-${port}..."
  local fw_out fw_try fw_ok=0
  for fw_try in 1 2 3; do
    fw_out=$(gcloud compute firewall-rules create "allow-socks5-${port}" \
      --project="$project_id" \
      --direction=INGRESS --priority=1000 --network=default \
      --action=ALLOW --rules="tcp:${port}" --source-ranges="$PROXY_SOURCE_RANGES" \
      --quiet 2>&1)
    if [ $? -eq 0 ] || printf '%s\n' "$fw_out" | grep -qiE 'already exists|alreadyExists'; then
      fw_ok=1
      break
    fi
    warn "[代理] 防火墙第 ${fw_try}/3 次失败，5 秒后重试"
    sleep 5
  done
  [ "$fw_ok" -eq 1 ] || warn "[代理] 防火墙规则未确认成功，1080 端口可能不可达"

  say "[代理] 等待 VM startup-script 和公网 IPv4..."
  local wait_round
  for wait_round in $(seq 1 24); do
    external_ip=$(gcloud compute instances describe "$instance_name" --project="$project_id" --zone="$zone" \
      --format='get(networkInterfaces[0].accessConfigs[0].natIP)' 2>/dev/null || true)
    if [ -n "$external_ip" ] && timeout 5 bash -c "cat < /dev/null > /dev/tcp/$external_ip/$port" 2>/dev/null; then
      break
    fi
    sleep 5
  done

  [ -n "${external_ip:-}" ] || { err "[代理] 未取得公网 IPv4"; return 1; }
  if ! timeout 6 bash -c "cat < /dev/null > /dev/tcp/$external_ip/$port" 2>/dev/null; then
    err "[代理] VM 已创建但 $external_ip:$port 仍不可达"
    return 1
  fi

  {
    printf 'PROXY_URL=socks5://%s:%s@%s:%s\n' "$proxy_user" "$proxy_pass" "$external_ip" "$port"
    printf 'PROXY_HOSTPORT=%s:%s\n' "$external_ip" "$port"
    printf 'PROXY_ADSPOWER=%s:%s:%s:%s\n' "$external_ip" "$port" "$proxy_user" "$proxy_pass"
  } > "$out_file"

  ok "[代理] 已创建 SOCKS5: $external_ip:$port"
  return 0
}

build_or_reuse_proxy() {
  local project_hint_file="$1" out_file="$2"

  if [ "$BUILD_PROXY" != "1" ]; then
    warn "[代理] BUILD_PROXY=$BUILD_PROXY，跳过 SOCKS5"
    return 1
  fi

  if [ "$REUSE_PROXY" = "1" ] && find_reusable_proxy "$out_file"; then
    return 0
  fi
  ...
  if [ -z "$project_id" ]; then
    say "[代理] 当前未设置默认 project，等待前台准备项目..."
    local n
    for n in $(seq 1 60); do
      if [ -s "$project_hint_file" ]; then
        project_id=$(head -n1 "$project_hint_file")
        break
      fi
      sleep 2
    done
  ...
  if [ -z "$project_id" ]; then
    project_id=$(gcloud projects list --format='value(projectId)' --limit=1 2>/dev/null | head -n1 || true)
  fi

  [ -n "$project_id" ] || { err "[代理] 无可用项目"; return 1; }
  create_proxy_vm "$project_id" "$out_file"
}
...
extract_vertex_keys_parallel() {
  ...
  local keydir
  keydir=$(mktemp -d /tmp/allinone_keys_XXXXXX)
  local workers=() pid

  say "并行处理 ${#projects[@]} 个项目：Vertex 必需 API 扫描/补齐 + Authorization key"
  for pid in "${projects[@]}"; do
    (
      if [ "$FULL_API_PASS" = "1" ] && project_billing_enabled "$pid" 2>/dev/null; then
        log "INFO" "[$pid] FULL_API_PASS=1：先执行一次全套 API 首轮"
        v60_enable_all_services "$pid" >&2 || true
      fi

      if ! ensure_vertex_key_apis "$pid" "allinone-Vertex提取前" >&2; then
        echo "[$pid] Vertex 必需 API 未就绪，跳过" >&2
        exit 0
      fi

      local_key=$(v27_setup_and_extract_aq_key "$pid" 1 2>/dev/null | \
        grep -oE 'AQ\.[A-Za-z0-9_.-]{20,}' | head -n1)
      if [ -n "$local_key" ]; then
        printf '%s\n' "$local_key" > "$keydir/$pid.key"
        echo "[$pid] Vertex key 提取成功: ${local_key:0:12}..." >&2
      else
  ...
  local w
  for w in "${workers[@]}"; do wait "$w" || true; done

  mapfile -t out_ref < <(cat "$keydir"/*.key 2>/dev/null | grep -oE 'AQ\.[A-Za-z0-9_.-]{20,}' | awk '!seen[$0]++')
  rm -rf "$keydir"
}

print_result() {
  local account="$1" proxy_ready="$2" proxy_url="$3" proxy_adspower="$4"
  shift 4
  local keys=("$@") k

  echo
  echo -e "${GREEN}${BOLD}====== Vertex Authorization key 结果 ======${NC}"
  ...
  if [ "$proxy_ready" = "1" ]; then
    ...
    echo "SOCKS5: 未创建/未复用成功"
  fi
}

upload_results() {
  local account="$1" proxy_url="$2"
  shift 2
  local keys=("$@")

  if [ "$NO_UPLOAD" = "1" ]; then
    warn "NO_UPLOAD=1：不上传任何 key/代理到远程面板"
    return 0
  fi

  if [ -z "$PANEL_TOKEN" ]; then
    err "NO_UPLOAD!=1 但 PANEL_TOKEN 为空；为避免泄露/误传，拒绝上传"
    return 1
  fi

  if [ -z "$proxy_url" ]; then
    err "没有可用 SOCKS5，拒绝执行 Key+Proxy 上传"
    return 1
  fi

  local keys_joined note payload resp
  keys_joined=$(printf '%s\n' "${keys[@]}")
  note="$account"

  payload=$(KEYS="$keys_joined" PROXY="$proxy_url" MODE="$UPLOAD_MODE" NOTE="$note" CBY="$CREATED_BY" python3 -c '
import json, os
p = {
    "keys": os.environ.get("KEYS", ""),
    "mode": os.environ.get("MODE", "pool"),
    "note": os.environ.get("NOTE", ""),
    "proxy_pref": os.environ.get("PROXY", ""),
}
try:
    c = int(os.environ.get("CBY", "0"))
    if c:
        p["created_by"] = c
except ValueError:
    pass
...
')
  ...
  say "上传 ${#keys[@]} 个 Vertex key + SOCKS5 到 ${PANEL_BASE}/api/v1/vertex-keys"
  resp=$(curl -sS --connect-timeout 15 --max-time 45 -X POST "${PANEL_BASE}/api/v1/vertex-keys" \
    -H "Authorization: Bearer ${PANEL_TOKEN}" \
    -H "Content-Type: application/json" \
    -d "$payload" 2>&1)

  if printf '%s\n' "$resp" | grep -q '"ok"[[:space:]]*:[[:space:]]*true'; then
    ok "上传成功"
    return 0
  fi

  err "上传未确认成功: $(printf '%s\n' "$resp" | cut -c1-800)"
  return 1
}

allinone_main() {
  require_allinone_commands || exit 1
  check_env || exit 1

  local account billing_id
  account=$(active_account)
  [ -n "$account" ] || { err "没有活动的 gcloud 登录账号"; exit 1; }
  say "Standalone Vertex + SOCKS5 v${ALLINONE_VERSION}"
  say "当前账号: $account"
  say "目标 Vertex key/项目数量: $NEED_PROJECTS"

  billing_id=$(choose_billing_account_auto || true)
  [ -n "$billing_id" ] || { err "未找到 Billing Account；可通过 BILLING_ID=xxxx 手动指定"; exit 1; }
  say "使用 Billing Account: $billing_id"

  local proxy_out project_hint
  proxy_out="/tmp/allinone_proxy_$$.env"
  project_hint="/tmp/allinone_project_$$.txt"
  rm -f "$proxy_out" "$project_hint"

  # Run proxy work in the background, overlapping it with project/key work.
  build_or_reuse_proxy "$project_hint" "$proxy_out" &
  local proxy_pid=$!

  local target_projects=()
  prepare_target_projects "$billing_id" target_projects || true
  ...
  printf '%s\n' "${target_projects[0]}" > "$project_hint"
  say "目标项目(${#target_projects[@]}): ${target_projects[*]}"

  local keys=()
  extract_vertex_keys_parallel keys "${target_projects[@]}"
  if [ "${#keys[@]}" -lt "$NEED_PROJECTS" ]; then
    warn "只提取到 ${#keys[@]}/${NEED_PROJECTS} 个 Vertex key"
  else
    ok "Vertex key 提取完成: ${#keys[@]} 个"
  fi

  say "等待 SOCKS5 后台任务..."
  wait "$proxy_pid" || true

  local proxy_ready=0 proxy_url="" proxy_hostport="" proxy_adspower=""
  if [ -s "$proxy_out" ]; then
    # shellcheck disable=SC1090
    source "$proxy_out"
    proxy_url="${PROXY_URL:-}"
    proxy_hostport="${PROXY_HOSTPORT:-}"
    proxy_adspower="${PROXY_ADSPOWER:-}"
  ...
  if [ "${#keys[@]}" -eq 0 ]; then
    err "没有提取到 Vertex key"
    [ "$proxy_ready" -eq 1 ] && warn "代理已存在: $proxy_url"
    exit 1
  fi

  print_result "$account" "$proxy_ready" "$proxy_url" "$proxy_adspower" "${keys[@]}"
